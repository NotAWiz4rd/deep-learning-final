...
for i in range(5):
    ...
    if device == "cuda":
        ...
        logits, loss = model(x, y)
    loss.backward()
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # gradient clipping to prevent shocking model updates
    optimizer.step()
    ...
    print(f"step {i:4d} | loss: {loss.item():.6f} | time: {dt*1000:.2f}ms | norm: {norm:.4f} | speed: {tokens_per_sec:.2f} tokens/sec")
