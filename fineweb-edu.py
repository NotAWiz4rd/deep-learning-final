...
# create the cache local directory if it doesn't exist
...
def tokenize(doc):
    # tokenises a single document and returns a numpy array of uint16 tokens
    tokens = [eot]
...
with mp.Pool(num_processors) as pool:
    ...
    for tokens in pool.imap(tokenize, fineweb, chunksize=16):
        ...
        if token_count + len(tokens) < shard_size:
            # yes, just add the tokens to the current shard
